...
flowchart TD
    Dispatcher[Диспетчер]
    Driver[Водитель]
    A1[Планирование рейсов]
    A2[Назначение на рейс]
    A3[Учет выполнения]
    D1[БД_Автобусов]
    D2[БД_Водителей]
    D3[БД_Рейсов]
    ...
    %% Основные потоки
    Dispatcher -->|Вход: запрос на рейс| A1
    
    A1 -->|Чтение: доступные ресурсы| D1
    A1 -->|Чтение: свободные водители| D2
    A1 -->|Выход: план рейса| A2
    
    A2 -->|Запись: назначение| D3
    A2 -->|Обновление: статус автобуса| D1
    A2 -->|Обновление: статус водителя| D2
    A2 -->|Выход: задание водителю| Driver
    
    Driver -->|Вход: отчет о рейсе| A3
    A3 -->|Запись: факт. данные| D3
    A3 -->|Обновление: пробег| D1
    A3 -->|Обновление: рабочие часы| D2
    A3 -->|Выход: отчет диспетчеру| Dispatcher
```
